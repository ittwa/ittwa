...
const NAV_STRUCTURE: NavGroup[] = [
  ...
  { type: "link", href: "/auction", label: "Auction" },
  {
    ...
    items: [
      ...
      { href: "/tags", label: "Tag Tracker", icon: "tag", desc: "Franchise & 5th-year tag history" },
      { href: "/data-check", label: "Data Check", icon: "shield", desc: "Sheet vs. Sleeper reconciliation" },
    ],
  },
  {
...
const MOBILE_NAV_SECTIONS: MobileNavSectionDef[] = [
  ...
  { label: "Contracts", items: [
    ...
    { href: "/tags", label: "Tag Tracker", icon: "tag", desc: "Franchise & 5th-year tags" },
    { href: "/data-check", label: "Data Check", icon: "shield", desc: "Sheet vs. Sleeper" },
  ]},
  { label: "History", items: [
...
function ThemeToggle() {
  ...
  // theme is initialized from the DOM class a blocking pre-paint script set
  // (see layout.tsx / theme-provider.tsx), which server-side rendering can't
  // see — so this icon can legitimately differ between the server-rendered
  // markup and the client's first render. suppressHydrationWarning on the
  // attributes that actually diverge avoids a false-positive hydration
  // warning without reintroducing a flash (no mounted-flag / effect swap).
  return (
    <button
      onClick={toggleTheme}
...
function NavIcon({ name, size = 20, color = "currentColor" }: { name: string; size?: number; color?: string }) {
  ...
  switch (name) {
    ...
    case "shield":
      return <svg {...p}><path d="M12 3l7 3v5c0 4.5-3 8-7 10-4-2-7-5.5-7-10V6l7-3z"/><path d="M9 12l2 2 4-4"/></svg>;
    case "tag":
      return <svg {...p}><path d="M12 2l9 9-9 9-9-9V2h9z"/><circle cx="7.5" cy="6.5" r="1.25" fill={color}/></svg>;
    default:
      return null;
  }
}
