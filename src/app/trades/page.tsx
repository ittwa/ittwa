...
export default async function TradesPage() {
  ...
  const trades: EnrichedTrade[] = completedTrades.map((t) => {
    const count = (tradeCounter.get(season) || 0) + 1;
    tradeCounter.set(season, count);
    const tradeId = `${season}${String(count).padStart(2, "0")}`;
    ...
    return { id: tradeId, created: t.created, week, season, sides };
  });
